...
import { Users, BookOpen, TrendingUp, Bell, Send, Trash2, Clock, FileText, Megaphone, Save, X, PlusCircle, CheckCircle, HelpCircle, Video, Eye, Award, LayoutDashboard, Menu, Search, LogOut, MessageSquare, ChevronRight, Settings, File, ArrowRight } from 'lucide-react';
                                                ...
                                                <div className="space-y-4">
                                                    ...
                                                    <div className="flex gap-4 mb-2">
                                                        ...
                                                        <label className="flex items-center cursor-pointer">
                                                            <input
                                                                ...
                                                                className="mr-2 text-indigo-600"
                                                            />
                                                            <span className="text-sm text-slate-700">PDF URL</span>
                                                        </label>
                                                    </div>
                                                    ...
                                                    {chapterNoteType === 'TEXT' ? (
                                                        ...
                                                        <div className="space-y-2">
                                                            <input
                                                                type="text"
                                                                placeholder="Paste PDF URL here (Google Drive, Dropbox, etc.)"
                                                                value={chapterNoteUrl}
                                                                onChange={(e) => setChapterNoteUrl(e.target.value)}
                                                                className="w-full p-2.5 border border-slate-300 rounded-lg text-sm bg-white text-slate-900"
                                                            />
                                                            <p className="text-xs text-slate-500">Provide a direct link to a PDF file.</p>
                                                        </div>
                                                    )}

                                                    <button
                                                        onClick={handleAddChapterNote}
                                                        className="w-full py-2.5 bg-indigo-600 text-white rounded-lg text-sm font-medium hover:bg-indigo-700 flex items-center justify-center"
                                                    >
                            ...
                            <div className="space-y-6">
                                <div className="grid grid-cols-2 gap-4">
                                    <div className="bg-slate-50 p-4 rounded-xl border border-slate-100 text-center">
                                        <p className="text-xs font-bold text-slate-500 uppercase mb-1">Chapters Bought</p>
                                        <p className="text-2xl font-bold text-indigo-600">
                                            {viewingStudent.purchasedChapterIds.length + (viewingStudent.purchasedCourseIds.length * 15)}
                                        </p>
                                    </div>
                                     <div className="bg-slate-50 p-4 rounded-xl border border-slate-100 text-center">
                                ...
                                <div>
                                    ...
                                    <div className="max-h-32 overflow-y-auto bg-slate-50 rounded-lg p-3 border border-slate-100 space-y-2">
                                        {viewingStudent.purchasedCourseIds.length > 0 && (
                                            viewingStudent.purchasedCourseIds.map(cid => (
                                                <div key={cid} className="text-xs font-semibold bg-indigo-100 text-indigo-700 px-2 py-1 rounded w-fit mb-1">
                                                    Full Course Access ({cid.includes('11') ? 'Class 11' : 'Class 12'})
                                                </div>
                                            ))
                                        )}
                                        {viewingStudent.purchasedChapterIds.length > 0 ? (
                                             viewingStudent.purchasedChapterIds.map(cid => {
                                                 ...
                                                 return ch ? (
                                                     <div key={cid} className="text-xs text-slate-600 flex items-center">
                                            ...
                                            viewingStudent.purchasedCourseIds.length === 0 && <p className="text-xs text-slate-400">No content purchased.</p>
                                        )}
                                    </div>
                                </div>
                            </div>
